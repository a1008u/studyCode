...
class ScreenViewApp {
    constructor() {console.log(`ScreenViewApp.constructor()`)}
    start() {
        let collectionModel = new ItemModel({Id: 0, DisplayName: "Select an Option:"})
        let itemCollection = new ItemCollection(ClickableItemCollection)
        let itemCollectionView = new ItemCollectionView({model: collectionModel, collection: itemCollection})
