...
const todosView = ($el, ...titleArr: string[]) => {
     ...
     */
    const render = (): void => {
        const $list = todoCollection.get().map(
            (todoModel) => {return TodoView(todoModel).render()}).join('')
    ...
    const attachEvents = (): void => {
        ...
        $todos.forEach(($todo) => {
            ...
            $checkbox.addEventListener('click', () => {action('toggleCompleted', id)})
            $removeBtn.addEventListener('click', () => {action('remove', id)})
        })
    }
