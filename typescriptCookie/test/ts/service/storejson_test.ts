...
let notConfirmJsonTrue = (): boolean => true;

let notConfirmJsonFaluse = (): boolean => false;

describe('【storejson】テスト_処理実行させない場合', () => {
  // before
  ...
  it('【localstorageとcookieに保存】正常系', () => {
    spyOn(localstorage, 'storeJsonInLocalStorage');
    spyOn(cookies, 'storeJsonInCookie');
    storejson.set(paramjson, 3, notConfirmJsonTrue);
    expect(localstorage.storeJsonInLocalStorage).not.toHaveBeenCalled();
    expect(cookies.storeJsonInCookie).not.toHaveBeenCalled();
  });
});

describe('【storejson】テスト_処理実行の場合', () => {
  // before
  let paramjson: paramjson = {};
  paramjson['test'] = ['testtt'];
  ...
  it('【localstorageとcookieに保存】正常系', () => {
    // execute
    storejson.set(paramjson, 3, notConfirmJsonFaluse);
  ...
  it('【localStorageの存在確認】 localstorageに存在する + cookieに存在しない', () => {
    // execute
    storejson.set(paramjson, 5, notConfirmJsonFaluse);
