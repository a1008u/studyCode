        ...
        [].forEach.call(document.getElementsByTagName("a"), (afterAtags) => {
            if (afterAtags.id == anchorId2) {
                ...
                if (afterquerys.length >= 50) {
                    // すでに?がある場合は、&を付ける
                    expect(afterAtag).toEqual('http://example2.com/?test=aaaaaa&test2=sssssss&test3=ddddddd&test=aaaaaa&test2=sssssss&test3=ddddddd');
                    expect(afterquerys).toEqual('test=aaaaaa&test2=sssssss&test3=ddddddd&test=aaaaaa&test2=sssssss&test3=ddddddd');
                }
                if (afterquerys.length <= 50) {
                    // ?がないので、?を付けているか確認
                    expect(afterAtag).toEqual('http://example2.com/?test=aaaaaa&test2=sssssss&test3=ddddddd');
